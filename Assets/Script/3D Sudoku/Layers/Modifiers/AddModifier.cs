...
{
    [System.Serializable]
    public class AddModifier : WFCModifier
    {
        [Tooltip("The NAME of the other layer to add on top.")]
        public string inputLayerName;
        // Simplified: Just add the non-bakcground pixels
        ...
        public override void Apply(WFCBlueprintLayer layer, List<WFCBlueprintLayer> context)
        {
            ...
            for (int x = 0; x < map.width; x++)
            {
                for (int y = 0; y < map.height; y++)
                {
                    ...
                    // If source is NOT background, paint it
